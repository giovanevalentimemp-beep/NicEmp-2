...
interface AuthContextValue {
  ...
  loading: boolean;
  isAdmin: boolean;
  localAdmin: LocalAdminSession | null;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [localAdmin, setLocalAdmin] = useState<LocalAdminSession | null>(() =>
    getLocalAdminSession(),
  );
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      profile,
      loading,
      isAdmin: profile?.role === "admin" || localAdmin?.role === "admin",
      localAdmin,
      signOut,
      refreshProfile,
    }),
    [loading, profile, session, localAdmin],
  );
